...
- **A cascade architecture: a cheap, fast first-pass filter ahead of the current three-score pipeline.** A pattern used in production fraud systems at scale — a lightweight embedding-based or rule-based check runs on every transaction first, cheaply separating clearly-clean traffic from anything ambiguous; only the ambiguous fraction is then run through the heavier, more expensive scoring this project builds. At this project's data volume, the full pipeline is already fast enough (57ms measured — Section 2.7) that a pre-filter has no latency benefit to offer. It becomes genuinely valuable at real production volume specifically to minimize false positives further by only spending the expensive, higher-fidelity check's "opinion" on traffic that actually warrants it — a real, informed next step for this architecture, not implemented here because the current volume doesn't yet justify the added complexity.
