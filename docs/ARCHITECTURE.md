...
- **Distribution shift is a real, unaddressed risk, named explicitly rather than assumed away.** Every number in this project — the model's precision/recall, the fairness disparity, the calibration curve — is only as valid as the assumption that the data it was measured on resembles the data it will actually see in production. Our synthetic data's feature distributions (COD rate, agent age spread, order-value range) were chosen to be plausible, not fit to Razorpay's real transaction distribution, which we do not have access to. If Razorpay's real agent-transaction data has a materially different shape — a different fraud base rate, a different COD prevalence, a different pincode risk spread — every threshold and every reported metric here would need to be re-derived against that real distribution before being trusted operationally, not simply assumed to transfer. This is the single largest reason the deployment story (Section 8 of docs/SPEC.md) starts with shadow mode rather than any form of live gating.
...
### 3.3 Why preference-fit is a heuristic, not a third trained model, and what grounds the heuristic itself
...
**The hypothesis behind the formula, stated explicitly rather than left implicit:** past behavior is a reasonable predictor of near-future behavior for the same individual, over a short window — the same general principle behind credit scoring's use of repayment history and loyalty/retention modeling's use of repeat-purchase patterns. user_past_over_budget_kept_rate operationalizes this narrowly: has this specific customer, in the past, kept orders that came in over their stated budget, rather than returning them? This is a hypothesis grounded in a general, well-established pattern in consumer-behavior modeling, not a citation to a specific study of this exact scenario — agentic-commerce preference-fit is too new a problem for such a study to exist yet. That distinction is worth being precise about: general grounding, not a formal citation, and treated with correspondingly lighter statistical rigor than the two trained scores.

**Where the two scores actually earn their place in the gating criteria, not just coexist:** fraud-risk and preference-fit are never used interchangeably — a high preference-fit score never overrides or discounts a high fraud-risk score (see Section 5's gating table: preference-fit is only ever consulted after both fraud-risk and intent-match have already cleared their own thresholds). Its sole function is disambiguating within the specific narrow case where fraud-risk is already low and intent-match has already failed — distinguishing "a mistake worth flagging for a human" from "a deviation this customer's own history suggests they'd welcome." Removing it would collapse that distinction back into a single default (either auto-block every intent deviation, or auto-approve all of them) — both worse than a labeled heuristic making the split.
...
- **A cascade architecture using user-action embeddings as a cheap, fast first-pass filter ahead of the current three-score pipeline.** A pattern used in production fraud systems at scale, raised directly in external review: represent a user's (or agent's) sequence of actions as a learned embedding, and use that lightweight representation to immediately separate clearly-normal behavior from anything anomalous — before any transaction reaches the heavier, more expensive scoring this project builds. Only the flagged, ambiguous fraction is then run through the full three-score pipeline, specifically to drive the false-positive rate down further by reserving the expensive, higher-fidelity check's "opinion" for the traffic that actually warrants it, rather than running every transaction through the same cost. At this project's data volume, the full pipeline is already fast enough (57ms measured — Section 2.7) that a pre-filter has no latency benefit to offer, and we have no real user-action sequence data to train an embedding model against. It becomes genuinely valuable at real production volume and with real behavioral data — a concrete, well-scoped next step for this architecture, not implemented here because neither precondition exists yet, not because the idea wasn't understood.
